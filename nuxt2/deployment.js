...
async function startAndWarmupRunningPM2() {
  ...
  try {
    ...
    if (await isRegistered(processName)) {
      ...
      // 새로 등록
      await execCommand(`pm2 start ${ecosystemConfigPath}`);
      console.log(
        colors.bold.green(
    ...
    // 2. PM2 프로세스 상태 확인
    await sleep(1000); // PM2 프로세스가 등록되는데 약간의 시간이 필요할 수 있음
  ...
  try {
    ...
    if (runningStarted) {
      ...
      if ((stabilityConfirmed && nginxRoutingConfirmed) || forceMainStop) {
        ...
        console.log(
          colors.bold.red(
            '<Error> 운영 서버 안정성 또는 Nginx 트래픽 라우팅이 확보되지 않아 메인 서버를 유지합니다. 수동으로 확인이 필요합니다.',
          ),
        );
