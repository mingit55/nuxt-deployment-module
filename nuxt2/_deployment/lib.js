...
async function getServicePort(isRunning) {
  try {
    ...
    const port =
      ecosystem.apps[0].env.PORT || (isRunning ? '1' + envPort : envPort);
...
async function verifyNuxtAppResources(host) {
  ...
  try {
    ...
    if (
      !htmlContent.includes('<div id="__nuxt">') &&
      !htmlContent.includes('<div id="__layout">')
    ) {
      console.log(
        colors.yellow(
      ...
      criticalScripts.map(async (scriptPath) => {
        try {
          const response = await new Promise((resolve, reject) => {
            const req = http.get(`http://${host}${scriptPath}`, (res) => {
              let data = '';
              res.on('data', (chunk) => {
                data += chunk;
              });
